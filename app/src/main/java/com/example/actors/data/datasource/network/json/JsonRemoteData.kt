...
class JsonRemoteData @Inject constructor(
    ...
    @Throws(Exception::class)
    fun fetchMovieTrailersJsonData(response: String): List<Trailer> {
        val jsonObject = JSONObject(response)
        ...
        for (i in 0 until resultsArray.length()) {
            val trailerObject = resultsArray.getJSONObject(i)
            val iso_639_1 = trailerObject.getString("iso_639_1")
            val iso_3166_1 = trailerObject.getString("iso_3166_1")
            val name = trailerObject.getString("name")
            val key = trailerObject.getString("key")
            val site = trailerObject.getString("site")
            val size = trailerObject.getInt("size")
            val type = trailerObject.getString("type")
            val official = trailerObject.getBoolean("official")
            val publishedAt = trailerObject.getString("published_at")
            val id = trailerObject.getString("id")

            val trailer = Trailer(iso_639_1, iso_3166_1, name, key, site, size, type, official, publishedAt, id)
